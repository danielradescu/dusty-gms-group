            ...
            >
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ __('dashboard.organized_by') }}
                        <span class="font-medium text-indigo-600 dark:text-indigo-400">
                            {{ $session->organizer->name ?? __('dashboard.unknown_organizer') }}
                        </span>
                    </p>
                </div>
                ...
                <div class="mt-4 space-y-1 text-sm text-gray-700 dark:text-gray-300">
                    <p class="flex items-center gap-2">
                        📅 <span>{{ $session->start_at->translatedFormat('l, d M Y') }}</span>
                    </p>
                    <p class="flex items-center gap-2">
                        ⏰ <span>{{ $session->start_at->format('H:i') }}</span>
                    </p>
                    <p class="flex items-center gap-2">
                        📍 <span>{!! \App\Helpers\TextHelper::linkify($session->location ?? __('dashboard.location_tbd')) !!}</span>
                    </p>
                ...
                @php
                    $confirmedCount = $session->registrations
                ...
                <div class="mt-4 flex justify-between items-center text-sm text-gray-700 dark:text-gray-300">
                    ...
                    @if($isHiddenForNow)
                        <span class="text-xs font-semibold px-2 py-1 rounded-full bg-yellow-100 text-yellow-800 border border-yellow-200
                            ...
                            🔒 {{ __('dashboard.hidden_until', ['date' => $session->delay_until->format('d M Y, H:i')]) }}
                        </span>
                    @else
                ...
                <!-- Action -->
                <x-link-button class="mt-6"
                               href="{{ route('game-session.interaction.show', $session->uuid) }}"
                               variant="primary">
